...
class PGAgent(BaseAgent):
    ...
    def train(self, obs, acs, rews_list, next_obs, terminals):
        ...
        # step 3:
        # (DONE) TODO: pass the calculated values above into the actor/policy's update,
    ...
    def estimate_advantage(self, obs, q_values):
        ...
        # (DONE) TODO: Estimate the advantage when nn_baseline is True
    ...
    def _discounted_return(self, rewards):
        ...
        discounts = [self.gamma ** idx for idx in range(len(rewards))]

        # 3) create a list where the entry at each index (t') is gamma^(t') * r_{t'}
        discounted_rewards = np.multiply(discounts, indices)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for start_time_index in range(len(rewards)): 
            ...
            indices = [] * (len(rewards) - start_time_index)

            # 2) create a list where the entry at each index (t') is gamma^(t'-t)
            discounts = [self.gamma ** idx for idx in range(len(indices))]
            ...
            discounted_rtg = np.multiply(discounts, indices)
